    ...
    for i = 1:length(DemandVar)
        ...
        for Nexc = 1:length(NameFileDemand)
            ...
            if strcmp(DemandVar{i},'Agricultural')
                ...
                % Checks 
                if ~UserData.Terminal
                    [ErrorMessage, DataV] = CheckData(UserData, DataV , DemandData.Date, DemandData.CodeBasin, [NameFileDemand{Nexc},' ',NameFileDemand{Nexc}], ProgressBar);
                else
                    [ErrorMessage, DataV] = CheckData(UserData, DataV , DemandData.Date, DemandData.CodeBasin, [NameFileDemand{Nexc},' ',NameFileDemand{Nexc}]);
                end
                if ~strcmp(ErrorMessage,'Successful Run'), if ~UserData.Terminal, close(ProgressBar),end, return, end
                ...
                % Checks 
                if ~UserData.Terminal
                    [ErrorMessage, DataM] = CheckData(UserData, DataM , DemandData.Date, CodeBasinM, [NameFileDemand{Nexc},' ',NameFileDemand{Nexc}], ProgressBar);
                else
                    [ErrorMessage, DataM] = CheckData(UserData, DataM , DemandData.Date, CodeBasinM, [NameFileDemand{Nexc},' ',NameFileDemand{Nexc}]);
                end
                if ~strcmp(ErrorMessage,'Successful Run'), if ~UserData.Terminal, close(ProgressBar),end, return, end
            ...
            nm          = length(UserData.Date);
            nn          = month(UserData.Date(1));
            ...
            elseif strcmp(DemandVar{i},'Domestic')
                ...
                Tmp             = repmat(DayMonths,length(UserData.ArcID),length(unique(year(UserData.Date))));
                Tmp             = Tmp(:,1:length(UserData.Date));
            ...
            SaveCsv_Total(NameFileCsv, UserData.ArcID, ValuesDemand, UserData.Date);

            %% Save Returns
            NameFileCsv = fullfile(  UserData.PathProject,'INPUTS', UserData.Scenarios,'Returns',...
                                    DemandVar{i},[NameFileDemand{Nexc},'.csv']);

            SaveCsv_Total(NameFileCsv, UserData.ArcID, ValuesDemand.*DemandData.PorReturns, UserData.Date);
